...
library Puzzle {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "", name: "Puzzle", typeId: RESOURCE_TABLE });`
  ...
  function decodeDynamic(
    ...
    bytes memory _blob
  ) internal pure returns (uint256[] memory picSeq) {
    uint256 _start;
    uint256 _end;
    unchecked {
      _end = _encodedLengths.atIndex(0);
    }
    picSeq = (SliceLib.getSubslice(_blob, _start, _end).decodeArray_uint256());
  }
   ...
   */
  function encodeLengths(uint256[] memory picSeq) internal pure returns (EncodedLengths _encodedLengths) {
    // Lengths are effectively checked during copy by 2**40 bytes exceeding gas limits
    unchecked {
      _encodedLengths = EncodedLengthsLib.pack(picSeq.length * 32);
    }
  }
   ...
   */
  function encodeDynamic(uint256[] memory picSeq) internal pure returns (bytes memory) {
    return abi.encodePacked(EncodeArray.encode((picSeq)));
  }
